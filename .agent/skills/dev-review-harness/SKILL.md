...
2. Dispatch `nextjs-developer` with `invoke_subagent` (`TypeName: nextjs-developer`, `Workspace: inherit`). The call is **asynchronous** — poll the subagent until it is `Idle` before continuing.
...
5. On `rejected`, hand `### Required changes` back to the developer and repeat from step 2 with the next
   cycle number. Stop after 3 rejections and escalate to the user.
